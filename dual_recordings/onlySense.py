...
#walnut: robotiq218
sensingMode = MODE_LINEAR_ROTATIONAL # MODE_INDIVIDUAL # MODE_INDIVIDUAL vs. MODE_LINEAR_ROTATIONAL
...
if SensorExist:
    ...
    ResultSavingDirectory = os.getcwd()
    # #%% Save Output
    import datetime
    ...
    output_file_2 = directory + '\\' + 'result_fft' +currDateTimeString + SavingFileName + '.csv'

    print(output_file)
    np.savetxt(output_file, sensor_1_data_history_first, delimiter=",")

    np.savetxt(output_file_2, fftStorage_sns_2_NS + fftStorage_sns_2_WE, delimiter=",")
