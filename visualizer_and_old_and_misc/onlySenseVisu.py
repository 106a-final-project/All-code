...
# import robotiq as r
import winsound
from visualizer import vectorFieldVisualiserQuiver as VisualizerVector
from visualizer import vectorFieldPixelVisualizerBoth as VisualizerPixelBoth
import matplotlib.pyplot as plt

runTime = 2000 # 20 for experiment, 4 for testing
...
#plt.ion()
#visualizer = VisualizerPixelBoth()
#visualizer.redraw()
...
while time.time() - tic < runTime:
    if SensorExist:
        ...
        tempSampledDataOrig = tempSampledData
        ...
        # if baseline.sum() < 100.0 and time.time() - tic > 2:
        #     print("updated baseline")
        #     baseline = tempSampledData
        tempSampledData = tempSampledData - baseline
        avg += tempSampledData
        if idx % 20 == 0:
            avg /= 20
            print(" dx ", avg[0, 2] - avg[0,3])# " d0 ", avg[0, 2] - avg[0,3])

            avg = tempSampledData
        idx += 1
        continue

        #print(f"temp sampled is {tempSampledData}")
        ...
        if snsIndex == 0:
            ...
            if read_count_sns_1_first == waitUntil_sns_1_first:
                ...
                #print(Differential_W_E)
                arrdxdy = [[[0, 0]] * 30] * 30
                #print("0", arrdxdy[0])
                #print("00", arrdxdy[0][0])
                #print(Differential_W_E.shape, Differential_N_S.shape)
                for i in range(30):
                    for j in range(30):
                        #a = Differential_N_S[i]
                        #print(arrdxdy[i][j][0])
                        arrdxdy[i][j][0] = Differential_N_S[i]
                        arrdxdy[i][j][1] = Differential_W_E[j]
                #visualizer.update_all(arrdxdy)
                #forimage = tempSampledDataOrig
                forimage = tempSampledDataOrig - baseline
                #print("feeding ", forimage)
                visualizer.update_all(np.abs(forimage/np.max(forimage)))#/np.max(forimage))
                visualizer.redraw()
